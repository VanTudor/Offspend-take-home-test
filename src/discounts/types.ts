import { IDiscountOffer, TPartnerMap, TDiscountModifier, TWrappedModifier, IWrappedDiscountModifierParams, IWrappedDiscountModifierParamsBase } from "../types";
  ...
  get(discountModifierMap: TPartnerMap<TWrappedModifier>, modifierName: string) {
    if (modifierName in discountModifierMap) {
      ...
      return new Proxy(discountModifierMap[modifierName], {
        apply(target: TWrappedModifier, thisArg: unknown, wrappedDiscountModifierParams: [IWrappedDiscountModifierParams]) {
            ...
            return new Proxy(target.apply(thisArg, wrappedDiscountModifierParams), {
              apply(target: TDiscountModifier, thisArg: unknown, discountOffer: [IDiscountOffer]) {
                ...
                discountOffer[0].discountInPercent = restrictions.reduce((prevValue, restrict) => restrict(prevValue), computedDiscount);
